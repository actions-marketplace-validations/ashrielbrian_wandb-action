...
def check_list(var, name):
    ...
    if var:
        ...
        ), f"{name} input must be a list of strings"
...
assert (
    run_id or github_sha
), "You must supply an input for either FILTER_GITHUB_SHA or RUN_ID.  Both of these inputs are not specified."

if secondary_sha and not github_sha:
    raise Exception(
        "If input FILTER_SECONDARY_SHA is supplied you must also supply an input for FILTER_GITHUB_SHA"
    )
...
if run_id:
    ...
    baseline_runs = api.runs(
        project_name,
        filters={"$and": [{"tags": {"$in": tags}}, {"name": {"$ne": f"{run_id}"}}]},
    )
    if github_sha:
        logging.info(
            "You have supplied both inputs FILTER_GITHUB_SHA and RUN_ID.  Runs matching FILTER_GITHUB_SHA will be ignored and only the run corresponding to RUN_ID will be returned."
        )
...
if not run_id and github_sha and secondary_sha:
    runs = api.runs(
        ...
        {
            "$and": [
                {"config.github_sha": f"{github_sha}"},
    ...
    baseline_runs = api.runs(
        project_name,
        {
            "$and": [
                ...
                {
                    "$or": [
                        {"config.github_sha": {"$ne": f"{github_sha}"}},
...
print(
    f"::set-output name=BOOL_COMPLETE::{True if finished_runs and not running_runs else False}"
)
...
def summarize_runs(runs, eval_category_label, debug, metrics=[], config_vars=[]):
    ...
    for run in runs:
        ...
        summary_dict["run.project"] = summary_dict.get("run.project", []) + [
            run.project
        ]
        summary_dict["github_sha"] = summary_dict.get("github_sha", []) + [
            run.config.get("github_sha")
        ]
        summary_dict["run.description"] = summary_dict.get("run.description", []) + [
            run.description
        ]

        for metric in metrics:
            summary_dict[metric] = summary_dict.get(metric, []) + [
                run.summary_metrics.get(metric)
            ]

        for var in config_vars:
            # configuration variables preceded with _ to avoid name collisions with metrics
            summary_dict[f"_{var}"] = summary_dict.get(f"_{var}", []) + [
                run.config.get(var)
            ]
